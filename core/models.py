...
class PartnerAccount(models.Model):
    user = models.OneToOneField(User, on_delete=models.CASCADE, related_name="partner_account")
    partner = models.OneToOneField(Partner, on_delete=models.CASCADE, related_name="account")

    def __str__(self):
        return f"{self.partner.name} ({self.user.get_full_name()})"
